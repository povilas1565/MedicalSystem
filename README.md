...
## ВВЕДЕНИЕ
Бекэнд приложения, сделанный для пациентов и центров и центров. 

### Использованный стек технологий:
- **Backend:** Java + SpringBoot + PostgreSQL + Hibernate + JPA Buddy
...
1.4) Проверка и обновление озданного аккаунта(метод PUT).
...
# 8. ImageController
8.1) Загрузка фото для профилей пользователей(метод POST).
Входные данные: файлы.
Выходные данные: добавления фоток к профилям.
8.2) Получение фоток профилей пользоателей(метод GET).
...
13.1) Проверка доступности тех или иных медработников по названиям центроа(метод POST).
...
21.24) Отмен запросов на приемы(метод DELETE).
Входные данные: role.
...
21.26) Приемы закончены(метод PUT)C
